"""
Script to traing a neural network in "resume" mode, given a starting epoch and a specific k partition
"""
...
def train(args, logger=None):
    ...
    print("Resuming experiment '{}' on time {}".format(args.exp_name, d_time))
    ...
    # Starting epoch
    starting_epoch = args.start_from

    # Create neural network model. If starting_epoch != 0, load last existing model for that partition
    network: DGCNN = None
    if starting_epoch != 0 and args.retrain == False:
        print("Loading pretrained model")
        network = neural_handler.load_model(args.checkpoint)
    ...
    # Load specific partition (k)
    k = args.start_partition
    (train_data, train_labels), (test_data, test_labels) = processor.load_partition(args.start_partition)
    # K Fold Cross Validation
    print("K-Fold Cross Validation: partition {}".format(k + 1))
    ...
    train_dset = SemanticSegmentation(points=train_data, labels=train_labels, args=args)
    test_dset = SemanticSegmentation(points=test_data, labels=test_labels, args=args, training=False)
    ...
    for epoch in range(starting_epoch, args.epochs):
        ...
        # Training loop
        print("Resuming training on epoch {}/{} and partition {}".format(epoch + 1, args.epochs, k + 1))
        network.train()
        ...
        async_confusion = AsyncConfusionMatrix(prediction_tensors, ground_truth_tensors, args.num_classes, logger, epoch + 1, k + 1, 'training')

        # Validation loop (test)
        print("Performing evaluation on epoch {}/{} and partition {}".format(epoch + 1, args.epochs, k + 1))
        ...
        # Calculate confussion matrix (asynchronous)
        async_confusion = AsyncConfusionMatrix(prediction_tensors, ground_truth_tensors, args.num_classes, logger, epoch + 1, k + 1, 'validation')
        ...
        log = log.format(k + 1, epoch + 1, args.epochs, train_epoch_loss, train_epoch_accuracy, test_epoch_loss, test_epoch_accuracy)
        print(log)

        # Register metrics on tensorboard
        logger.add_scalar('loss/train', train_epoch_loss, epoch + 1, kfold=True, k=k+1)
        logger.add_scalar('loss/test', test_epoch_loss, epoch + 1, kfold=True, k=k+1)
        logger.add_scalar('accuracy/train', train_epoch_accuracy, epoch + 1, kfold=True, k=k+1)
        logger.add_scalar('accuracy/test', test_epoch_accuracy, epoch + 1, kfold=True, k=k+1)

        # Add pairs of scalars, so it is possible to compare them
        logger.add_scalars('loss', {
            'train': train_epoch_loss,
            'test': test_epoch_loss
        }, epoch + 1, kfold=True, k=k+1)
        ...
        logger.add_scalars('iou/train', iou_train, epoch + 1, kfold=True, k=k+1)
        ...
        logger.add_scalars('iou/test', iou_test, epoch + 1, kfold=True, k=k+1)
        ...
        logger.add_text('points_per_class/train', text=str(train_num_points_class), kfold=True, k=k)
        logger.add_text('points_per_class/test', text=str(test_num_points_class), kfold=True, k=k)

    # Print metrics
    log = "Finished resumed experiment for partition {} starting from epoch {}"
    log = log.format(k + 1, starting_epoch + 1)
    print(log)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, help="The batch size of the experiment (default 2)", default=2)
    ...
    # Number of points
    parser.add_argument('--num_points', type=int, help="Number of points used in the experiment", default=4096)
    ...
    # Transfer learning
    parser.add_argument('--retrain', type=bool, help="To retrain a model from a checkpoint", default=True)
    ...
    # Epoch to start from
    parser.add_argument('--start_from', type=int, help="The epoch to start from when resuming training", default=1)

    # The partition to train and validate with
    parser.add_argument('--start_partition', type=int, help="The partition to use in this training", default=0)
